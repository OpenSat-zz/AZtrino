...
void CAudioPlayerGui::cutNeutrino()
{
	printf("Cut neutrino\n");
...
void CAudioPlayerGui::restoreNeutrino()
{
...
int CAudioPlayerGui::show()
{
	...
	while(loop)
	{
		...
		if ((playback->IsEOF()) && (!m_playlist.empty()) && (m_state == CAudioPlayerGui::PLAY))
			{
			dprintf(1,"End of file, playing next\n");
			playNext();
			}
...
void CAudioPlayerGui::play(unsigned int pos)
{
	...
	playback->Start((char *)m_curr_audiofile.Filename.c_str(), 0, 0, 0, true,0);
