...
void CTimeOSD::drawcustom(const char* value)
{
//	char cDisplayTime[8+1] = "PAUSED";
	int xstart = g_settings.screen_StartX + 10;
	int xend = g_settings.screen_EndX - 25;
	int y = g_settings.screen_StartY + 15;
	int height = g_Font[TIMEOSD_FONT]->getHeight();
	int width = g_Font[TIMEOSD_FONT]->getRenderWidth(value) + 10;
	fb_pixel_t color1 = COL_MENUCONTENT_PLUS_0;
	fb_pixel_t color2 = COL_MENUCONTENT;
	frameBuffer->paintBoxRel(xend - width - 15, y, width + 15, height, color1);
	g_Font[TIMEOSD_FONT]->RenderString(xend - width,  y + height, width + 20, value, color2);
}

void CTimeOSD::hidecustom()
{
	int xstart = g_settings.screen_StartX + 10;
	int xend = g_settings.screen_EndX - 25;
	int y = g_settings.screen_StartY + 15;
	...
	//frameBuffer->paintBackgroundBoxRel(m_xstart-10, m_y - 10 , m_xend - (g_settings.mode_clock ? 35 : 0), m_height + 15);
	frameBuffer->paintBackgroundBoxRel(xstart-2, y , xend - (g_settings.mode_clock ? 35 : 0), height + 15);
	visible=false;
	timescale->reset();
}