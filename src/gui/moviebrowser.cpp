...
int CMovieBrowser::exec(const char* path)
{
	...
	int m_LastMode = (CNeutrinoApp::getInstance()->getLastMode() | NeutrinoMessages::norezap);

	int fd;
	fd = open("/proc/player", O_RDWR);
	if (fd > 0)
	{
		write(fd, "0", 1);
		close(fd);
	...
	while (loop)
	{
		...
		if(result == false)
		{
			...
			else if(msg == CRCInput::RC_ok)
			{
				...
				if(m_movieSelectionHandler != NULL)
				{
//Opensat tmp disable until fixed
...
void CMovieBrowser::refreshFoot(void) 
{
	...
	//Opensat - Added Mute button
	std::string mute_text = g_Locale->getText(LOCALE_FILEBROWSER_DELETE);
	int xpos3 = xpos2 + width1;
	m_pcWindow->paintIcon(NEUTRINO_ICON_BUTTON_MUTE_SMALL, m_cBoxFrame.iX+xpos3, m_cBoxFrame.iY+m_cBoxFrameFootRel.iY + (ADD_FOOT_HEIGHT>>1));
	m_pcFontFoot->RenderString(m_cBoxFrame.iX+xpos3 + 30, m_cBoxFrame.iY+m_cBoxFrameFootRel.iY + m_cBoxFrameFootRel.iHeight + 4 , width4-30, mute_text.c_str(), (CFBWindow::color_t)color, 0, true); // UTF-8
...
//Opensat disabled this as its only in german
//    mainMenu.addItem(GenericMenuSeparatorLine);
