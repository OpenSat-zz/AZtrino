...
int CScanTs::exec(CMenuTarget* parent, const std::string & actionKey)
{
	...
	videoDecoder->ShowPicture(DATADIR "/neutrino/icons/scan.mvi");
...
//printf("[neutrino] scan_mode %d TP_freq %s TP_rate %s TP_fec %d TP_pol %d\n", get_set.scan_mode, get_set.TP_freq, get_set.TP_rate, get_set.TP_fec, get_set.TP_pol);
	...
	else if(blindscan)
	{
		...
		success = g_Zapit->scan_blind(get_set.scan_mode,atoi(get_set.TP_startfreq), atoi(get_set.TP_endfreq), atoi(get_set.TP_rate), get_set.TP_pol, get_set.TP_tone);
	}
	else
