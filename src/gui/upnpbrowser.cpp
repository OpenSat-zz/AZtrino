...
const struct button_label RescanButton = { NEUTRINO_ICON_BUTTON_BLUE,
		LOCALE_UPNPBROWSER_RESCAN };
const struct button_label StopButton = { NEUTRINO_ICON_BUTTON_YELLOW,
		LOCALE_AUDIOPLAYER_STOP };
const struct button_label PUpButton = { NEUTRINO_ICON_BUTTON_RED,
		LOCALE_FILEBROWSER_NEXTPAGE };
const struct button_label PDownButton = { NEUTRINO_ICON_BUTTON_GREEN,
		LOCALE_FILEBROWSER_PREVPAGE };

static CUpnpBrowserGui::state playstate;
...
int CUpnpBrowserGui::exec(CMenuTarget* parent, const std::string & /*actionKey*/) {
	...
	m_width = (g_settings.screen_EndX - g_settings.screen_StartX)
	...
	m_listmaxshow = (m_height - m_info_height - m_title_height - m_theight - 2
			* m_buttonHeight) / (m_fheight);
	m_height = m_theight + m_info_height + m_title_height + 2 * m_buttonHeight
			+ m_listmaxshow * m_fheight; // recalc height

	m_x = (((g_settings.screen_EndX - g_settings.screen_StartX) - (m_width
			+ ConnectLineBox_Width)) / 2) + g_settings.screen_StartX
			+ ConnectLineBox_Width;
	m_y
			= (((g_settings.screen_EndY - g_settings.screen_StartY) - m_height)
					/ 2) + g_settings.screen_StartY;
...
void CUpnpBrowserGui::splitProtocol(std::string &protocol, std::string &prot,
		std::string &network, std::string &mime, std::string &additional) {
...
std::vector<UPnPEntry> *CUpnpBrowserGui::decodeResult(std::string result) {
	...
	for (node = root->GetChild(); node; node = node->GetNext()) {
		...
		if (!strcmp(node->GetType(), "container")) {
			...
			UPnPEntry entry = { id, isdir, title, artist, album, children,
					resources, -1 };
...
void CUpnpBrowserGui::selectDevice() {
	...
	CHintBox *scanBox = new CHintBox(LOCALE_MESSAGEBOX_INFO, g_Locale->getText(
			LOCALE_UPNPBROWSER_SCANNING)); // UTF-8
	scanBox->paint();
#if 0
	try {
...
#endif
	m_devices = m_socket->Discover(
			"urn:schemas-upnp-org:service:ContentDirectory:1");
	scanBox->hide();

	if (!m_devices.size()) {
		ShowLocalizedMessage(LOCALE_MESSAGEBOX_INFO,
				LOCALE_UPNPBROWSER_NOSERVERS, CMessageBox::mbrBack,
				CMessageBox::mbBack, "info.raw");
		delete scanBox;
		return;
	}
	...
	while (loop) {
		...
		else if (msg_repeatok == CRCInput::RC_down && m_selecteddevice + 1
				< m_devices.size()) {
			m_selecteddevice++;
		...
		else if (msg == NeutrinoMessages::RECORD_START || msg
				== NeutrinoMessages::ZAPTO || msg
				== NeutrinoMessages::STANDBY_ON || msg
				== NeutrinoMessages::SHUTDOWN || msg
				== NeutrinoMessages::SLEEPTIMER) {
			loop = false;
...
#endif
		results = m_devices[m_selecteddevice].SendSOAP(
		...
		if ((entries != NULL) && (!(*entries)[0].isdir)) {
			...
			if (preferred != -1) {
				...
				if (mime == "audio/mpeg") {
					...
					m_playing_entry_is_shown = false;
					CAudiofile mp3((*entries)[0].resources[preferred].url,
							CFile::FILE_MP3);
					CAudioPlayer::getInstance()->play(&mp3,
							g_settings.audioplayer_highprio == 1);
					return;
...
bool CUpnpBrowserGui::selectItem(std::string id) {
	...
	while (loop) {
		...
		if (rchanged) {
			...
			attribs.push_back(UPnPAttribute("BrowseFlag",
			...
			if (!(*entries)[selected - index].isdir) {
				...
				if (preferred != -1) {
					...
					if ((mime == "image/gif") || (mime == "image/jpeg")) {
						...
						CHTTPTool httpTool;
						httpTool.downloadFile((*entries)[selected - index].resources[preferred].url,"/tmp/filetmpUPNP.jpg", 20);

						viewer->Cleanup();
						viewer->ShowImage("/tmp/filetmpUPNP.jpg", false);
						...
						remove("/tmp/filetmpUPNP.jpg");
						delete viewer;

					}

					else if ((mime == "video/x-msvideo") || (mime == "video/x-matroska") ||
							(mime == "audio/mpeg") || (mime == "audio/x-vorbis+ogg")) {
						...
						int startposition;
						int speed=1;
						const char * filename =	(*entries)[selected - index].resources[preferred].url.c_str();

						unsigned short g_vpid = 0;
						unsigned short g_vtype = 0;
						unsigned int g_currentapid = 0, g_currentac3 = 0;
						int g_length = 0;
						//extern cVideo * videoDecoder;
						...
						playstate = CUpnpBrowserGui::PREPARING;
						playback->Open(PLAYMODE_FILE);

						do
						{
							if (playstate < CUpnpBrowserGui::PLAY)
							{
								if (!playback->Start((char *) filename, g_vpid,	g_vtype, g_currentapid, g_currentac3, g_length)) {
									playstate = CUpnpBrowserGui::PLAY;
									playback->Close();
								} else {
									playstate = CUpnpBrowserGui::PLAY;
									CVFD::getInstance()->ShowIcon(VFD_ICON_PLAY, true);

									if (startposition >= 0)//FIXME no jump for file at start yet
										playback->SetPosition(startposition, true);

								}

								if ((mime == "audio/mpeg") || (mime == "audio/x-vorbis+ogg")) {
									std::string msg=(*entries)[selected - index].title.c_str();
									msg+="\n";
									msg+=(*entries)[selected - index].artist.c_str();
									msg+="\n";
									msg+=(*entries)[selected - index].album.c_str();
									msg+="\n";
									msg+=(*entries)[selected - index].resources[preferred].duration.c_str();
									viewer->Cleanup();
									viewer->ShowImage(DATADIR "/neutrino/icons/music.jpg", false);

									infobox = new CHintBox(LOCALE_MESSAGEBOX_INFO, msg.c_str());
									infobox->paint();
								}
							 }
							...
							dprintf(1,"Got msg: %i %i  - compare %i\n\n", msg, data,  CRCInput::RC_minus);

							if ((msg == (neutrino_msg_t) g_settings.mpkey_stop) || (msg == CRCInput::RC_home))
									{
										//exit play
										playstate = CUpnpBrowserGui::STOPPED;
										dprintf(0,"Key stopped pressed.\n");
									}

							else if (msg == (neutrino_msg_t) g_settings.mpkey_pause) {
								if (playstate >= CUpnpBrowserGui::PAUSE)
								{
									speed = 1;
								...
								{
									m_frameBuffer->paintIcon("start.mvi",0,0);
									speed = 0;
									playback->SetSpeed(speed);
									playstate = CUpnpBrowserGui::PAUSE;
								}
								...
								playback->SetSpeed(speed);
								playstate = CUpnpBrowserGui::FF;

								char tmpmsg[10];
								sprintf(tmpmsg, "<< %d X", speed);

							} else if ((msg == CRCInput::RC_minus) ||(msg == CRCInput::RC_plus) )
							{
								...
								dprintf(0,"Set Volume: %i \n",g_settings.current_volume);

							}else if (msg == 305) //Finish song or movie
									{
										playstate=CUpnpBrowserGui::STOPPED;
									}


						} while (playstate >= CUpnpBrowserGui::PLAY);
						...
						dprintf(0,"Format not implemented\n");
					}
		...
		if (m_folderplay && (CAudioPlayer::getInstance()->getState()
				== CBaseDec::STOP))
			playnext();
	}
	if (entries)
...
void CUpnpBrowserGui::paintDevicePos(unsigned int pos) {
	...
	int
			w =
					g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->getRenderWidth(
							name) + 5;
	g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(m_x
			+ 10, ypos + m_fheight, m_width - 30 - w, num, color, m_fheight,
			true); // UTF-8
	g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(m_x
			+ m_width - 15 - w, ypos + m_fheight, w, name, color, m_fheight,
			true); // UTF-8
}

//------------------------------------------------------------------------

void CUpnpBrowserGui::paintItemPos(std::vector<UPnPEntry> *entry,
		unsigned int pos, unsigned int selected) {
	int ypos = m_y + m_title_height + m_theight + pos * m_fheight;
	uint8_t color;
	fb_pixel_t bgcolor;
	...
	g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(m_x
			+ m_width - 15 - w, ypos + m_fheight, w, info, color, m_fheight);
	g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(m_x
			+ 30, ypos + m_fheight, m_width - 50 - w, name, color, m_fheight,
			true); // UTF-8
...
void CUpnpBrowserGui::paintDevice() {
	...
	CVFD::getInstance()->showMenuText(0,
			m_devices[m_selecteddevice].friendlyname.c_str(), -1, true);

	// Info
	m_frameBuffer->paintBoxRel(m_x, m_y, m_width, m_title_height - 10,
			COL_MENUCONTENT_PLUS_6, c_rad_mid);
	m_frameBuffer->paintBoxRel(m_x + 2, m_y + 2, m_width - 4, m_title_height
			- 14, COL_MENUCONTENTSELECTED_PLUS_0, c_rad_mid);
	...
	if (xstart < 10)
		xstart = 10;
	g_Font[SNeutrinoSettings::FONT_TYPE_MENU]->RenderString(m_x + xstart, m_y
			+ 4 + 1 * m_mheight, m_width - 20, tmp, COL_MENUCONTENTSELECTED, 0,
			true); // UTF-8
	...
	if (xstart < 10)
		xstart = 10;
	g_Font[SNeutrinoSettings::FONT_TYPE_MENU]->RenderString(m_x + xstart, m_y
			+ 4 + 2 * m_mheight, m_width - 20, tmp, COL_MENUCONTENTSELECTED, 0,
			true); // UTF-8
	// third line
	...
	if (xstart < 10)
		xstart = 10;
	g_Font[SNeutrinoSettings::FONT_TYPE_MENU]->RenderString(m_x + xstart, m_y
			+ 4 + 3 * m_mheight, m_width - 20, tmp, COL_MENUCONTENTSELECTED, 0,
			true); // UTF-8

	// Head
	tmp = g_Locale->getText(LOCALE_UPNPBROWSER_HEAD);
	m_frameBuffer->paintBoxRel(m_x, m_y + m_title_height, m_width, m_theight,
			COL_MENUHEAD_PLUS_0, c_rad_mid, CORNER_TOP);
	m_frameBuffer->paintIcon(NEUTRINO_ICON_UPNP, m_x + 7, m_y + m_title_height
			+ 6);
	g_Font[SNeutrinoSettings::FONT_TYPE_MENU_TITLE]->RenderString(m_x + 35, m_y
			+ m_theight + m_title_height + 0, m_width - 45, tmp, COL_MENUHEAD,
			0, true); // UTF-8
	ypos = m_y + m_title_height;
	if (m_theight > 26)
		ypos = (m_theight - 26) / 2 + m_y + m_title_height;
	m_frameBuffer->paintIcon(NEUTRINO_ICON_BUTTON_DBOX, m_x + m_width - 30,
			ypos);
#if 0
	...
	m_frameBuffer->paintBoxRel(m_x + m_width - 15, ypos, 15, sb,
			COL_MENUCONTENT_PLUS_1);

	int sbc = ((m_devices.size() - 1) / m_listmaxshow) + 1;
	int sbs = ((m_selecteddevice) / m_listmaxshow);

	m_frameBuffer->paintBoxRel(m_x + m_width - 13, ypos + 2 + sbs * (sb - 4)
			/ sbc, 11, (sb - 4) / sbc, COL_MENUCONTENT_PLUS_3);

	// Foot
	top = m_y + (m_height - m_info_height - 2 * m_buttonHeight);

	int ButtonWidth = (m_width - 20) / 4;
	m_frameBuffer->paintBoxRel(m_x, top, m_width, 1 * m_buttonHeight,
			COL_MENUHEAD_PLUS_0, c_rad_mid, CORNER_BOTTOM);
	m_frameBuffer->paintHLine(m_x, m_x + m_width, top,
			COL_INFOBAR_SHADOW_PLUS_0);
	::paintButtons(m_frameBuffer,
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL], g_Locale, m_x
					+ 10, top + 4, ButtonWidth, 1, &RescanButton);
...
void CUpnpBrowserGui::paintItem(std::vector<UPnPEntry> *entry,
		unsigned int selected, unsigned int max, unsigned int offset) {
	std::string tmp;
	std::stringstream ts;
	int w, xstart, ypos, top;
	...
	CVFD::getInstance()->showMenuText(0, (*entry)[selected].title.c_str(), -1,
			true);
	...
	m_frameBuffer->paintBoxRel(m_x + 2, m_y + 2, m_width - 4, m_title_height
			- 14, COL_MENUCONTENTSELECTED_PLUS_0);

	// first line
	ts << "Resources: " << (*entry)[selected].resources.size() << " Selected: "
			<< preferred + 1 << " ";
	tmp = ts.str();

	if (preferred != -1)
		tmp = tmp + "Duration: "
				+ (*entry)[selected].resources[preferred].duration;
	else
	...
	if ((*entry)[selected].isdir)
		...
		if (preferred != -1) {
			...
			splitProtocol((*entry)[selected].resources[preferred].protocol,
					proto, network, mime, info);
	...
	if ((sbc > 0) && (sbc > sb - 4))
		sbh = 2;
	m_frameBuffer->paintBoxRel(m_x + m_width - 13, ypos + 2 + sbs * ((sb - 4)
			/ sbc + sbh), 11, (sb - 4) / sbc + sbh, COL_MENUCONTENT_PLUS_3);

	// Foot buttons
	top = m_y + (m_height - m_info_height - 2 * m_buttonHeight);
	int ButtonWidth = (m_width - 20) / 4;
	m_frameBuffer->paintBoxRel(m_x, top, m_width, 1 * m_buttonHeight + 2,
			COL_MENUHEAD_PLUS_0);
	m_frameBuffer->paintHLine(m_x, m_x + m_width, top,
			COL_INFOBAR_SHADOW_PLUS_0);
	::paintButtons(m_frameBuffer,
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL], g_Locale, m_x
					+ 10, top + 4, ButtonWidth, 1, &StopButton);
	::paintButtons(m_frameBuffer,
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL], g_Locale, m_x
					+ ButtonWidth + 10, top + 4, ButtonWidth, 1, &PUpButton);
	::paintButtons(m_frameBuffer,
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL], g_Locale, m_x
					+ 2 * ButtonWidth + 10, top + 4, ButtonWidth, 1,
			&PDownButton);

	m_frameBuffer->paintIcon(NEUTRINO_ICON_BUTTON_OKAY, m_x + 3 * ButtonWidth
			+ 10, top + 1);
	g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->RenderString(m_x + 3
			* ButtonWidth + 40, top + 19 + 4, ButtonWidth - 40,
			g_Locale->getText(LOCALE_AUDIOPLAYER_PLAY), COL_INFOBAR, 0, true); // UTF-8
}

//------------------------------------------------------------------------

void CUpnpBrowserGui::paintDetails(std::vector<UPnPEntry> *entry,
		unsigned int index, bool use_playing) {
	// Foot info
	int top = m_y + (m_height - m_info_height - 1 * m_buttonHeight) + 2;

	if ((!use_playing) && ((*entry)[index].isdir)) {
		m_frameBuffer->paintBackgroundBoxRel(m_x, top + 2, m_width, 2
				* m_buttonHeight);
	} else {
		//		char cNoch[50]; // UTF-8
		//		char cSeit[50]; // UTF-8
		...
		if (use_playing) {
			if (!m_playing_entry_is_shown) {
				m_frameBuffer->paintBoxRel(m_x, top + 2, m_width - 2, 2
						* m_buttonHeight, COL_MENUCONTENTDARK_PLUS_0);
				m_playing_entry_is_shown = true;
				g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(
						m_x + 4, top + 1 * m_buttonHeight + 4, m_x + m_width
								- 8, m_playing_entry.title + " - "
								+ m_playing_entry.artist, COL_INFOBAR, 0, true); // UTF-8
				g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(
						m_x + 4, top + 2 * m_buttonHeight + 4, m_x + m_width
								- 8, m_playing_entry.album, COL_INFOBAR, 0,
						true); // UTF-8
			}
		} else {
			if (entry == NULL)
			...
			m_playing_entry_is_shown = false;
			g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(
					m_x + 4, top + 1 * m_buttonHeight + 4, m_x + m_width - 8,
					(*entry)[index].title + " - " + (*entry)[index].artist,
					COL_INFOBAR, 0, true); // UTF-8
			g_Font[SNeutrinoSettings::FONT_TYPE_FILEBROWSER_ITEM]->RenderString(
					m_x + 4, top + 2 * m_buttonHeight + 4, m_x + m_width - 8,
					(*entry)[index].album, COL_INFOBAR, 0, true); // UTF-8
		}
...
void CUpnpBrowserGui::paintItem2DetailsLine(int pos, unsigned int /*ch_index*/) {
	...
	// Clear
	m_frameBuffer->paintBackgroundBoxRel(xpos, m_y + m_title_height,
			ConnectLineBox_Width, m_height + m_info_height - (m_y
					+ m_title_height));
	if (pos < 0) {
		m_frameBuffer->paintBackgroundBoxRel(m_x, m_y + (m_height
				- m_info_height - 1 * m_buttonHeight) + 2, m_width,
				m_info_height);
	}
	// paint Line if detail info (and not valid list pos)
	if (pos >= 0) {
		// 1. col thick line
		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 4, ypos1, 4,
				m_fheight, col1);
		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 4, ypos2, 4,
				m_info_height, col1);

		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 15, ypos1a, 4,
				ypos2a - ypos1a, col1);

		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 15, ypos1a,
				12, 4, col1);
		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 15, ypos2a,
				12, 4, col1);

		// 2. col small line
		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 4, ypos1, 1,
				m_fheight, col2);
		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 4, ypos2, 1,
				m_info_height, col2);

		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 15, ypos1a, 1,
				ypos2a - ypos1a + 4, col2);

		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 15, ypos1a,
				12, 1, col2);
		m_frameBuffer->paintBoxRel(xpos + ConnectLineBox_Width - 12, ypos2a, 8,
				1, col2);

		// -- small Frame around infobox
		m_frameBuffer->paintBoxRel(m_x, ypos2, 2, m_info_height, col1);
		m_frameBuffer->paintBoxRel(m_x + m_width - 2, ypos2, 2, m_info_height,
				col1);
		m_frameBuffer->paintBoxRel(m_x, ypos2, m_width - 2, 2, col1);
		m_frameBuffer->paintBoxRel(m_x, ypos2 + m_info_height - 2, m_width - 2,
				2, col1);
	}
}
...
void CUpnpBrowserGui::updateTimes(const bool force) {
	...
	if (CAudioPlayer::getInstance()->getState() != CBaseDec::STOP) {
		...
		char play_time[8];
		snprintf(play_time, 7, "%ld:%02ld", m_time_played / 60, m_time_played
				% 60);
		char tmp_time[] = "000:00";
		int
				w =
						g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->getRenderWidth(
								tmp_time);

		if (updatePlayed) {
			paintDetails(NULL, 0, true);
			top = m_y + (m_height - m_info_height - 1 * m_buttonHeight)
					+ m_buttonHeight + 4;
			m_frameBuffer->paintBoxRel(m_x + m_width - w - 15, top + 1, w + 4,
					m_buttonHeight, COL_MENUCONTENTDARK_PLUS_0);
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->RenderString(
					m_x + m_width - w - 11, top + 1 + m_buttonHeight, w,
					play_time, COL_MENUHEAD);
		}
	}
}

void CUpnpBrowserGui::cutNeutrino()
{
	printf("Cut neutrino\n");
...
void CUpnpBrowserGui::restoreNeutrino()
{
