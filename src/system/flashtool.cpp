...
/*	//Opensat changed filename because of tar
	system("mkdir /tmp/ram");
	system("mount -t tmpfs tmpfs -o size=125M /tmp/ram/");
	std::string filename2 = "/tmp/ram/flash.jffs2";
*/
...
/*	//Opensat create tar file for webif flashing
	std::string command = "tar -cvzf /home/root/image.tgz flash.jffs2 -C /tmp/ram";
	system(command.c_str());
	system("rm /tmp/ram/flash.jffs2");
	system("mv /home/root/image.tgz /tmp/ram/flash.jffs2");
	system("umount -f /tmp/ram");
	system("rmdir /tmp/ram");
*/
	if(statusViewer)
	{
...
bool CFlashTool::program( const std::string & filename, int globalProgressEndErase, int globalProgressEndFlash )
{
	...
	//Opensat added tar unpacking
	std::string command = "tar -xvf "+filename+" -C /tmp/";
	system(command.c_str());
	std::string filename2 = "/tmp/flash.jffs2";

	if( (fd1 = open( filename2.c_str(), O_RDONLY )) < 0 )//Opensat Changed to filename2
	{
		ErrorMessage = g_Locale->getText(LOCALE_FLASHUPDATE_CANTOPENFILE);
		return false;
	}
