...
#define OPTIONS_RAW_SPDIF_OPTION_COUNT 2
const CMenuOptionChooser::keyval OPTIONS_RAW_SPDIF_OPTIONS[OPTIONS_RAW_SPDIF_OPTION_COUNT] =
{
        { 0, NONEXISTANT_LOCALE, "PCM"	},
        { 1, NONEXISTANT_LOCALE, "RAW"	}
};
...
{
	{ 0, LOCALE_VIDEOMENU_VIDEOFORMAT_43         },
	{ 1, LOCALE_VIDEOMENU_VIDEOFORMAT_169        }
};

#define VIDEOMENU_43MODE_OPTION_COUNT 5
const CMenuOptionChooser::keyval VIDEOMENU_43MODE_OPTIONS[VIDEOMENU_43MODE_OPTION_COUNT] = 
{
	...
	{ DISPLAY_AR_MODE_PILLARBOX, NONEXISTANT_LOCALE, "Pillarbox"	},
...
{
	{ 0, NONEXISTANT_LOCALE, "480p 60Hz"	},
	{ 1, NONEXISTANT_LOCALE, "576p 50Hz"	},
	{ 2, NONEXISTANT_LOCALE, "720p 50Hz"	},
	{ 3, NONEXISTANT_LOCALE, "720p 60Hz"	},
	{ 4, NONEXISTANT_LOCALE, "1080i 50Hz"	},
	{ 5, NONEXISTANT_LOCALE, "1080i 60Hz"	},
	{ 6, NONEXISTANT_LOCALE, "1080p 50Hz"	},
	{ 7, NONEXISTANT_LOCALE, "1080p 60Hz"	}
};
...
//	addItem(new CMenuForwarder(LOCALE_VIDEOMENU_ENABLED_MODES, true, NULL, menu));
...
#endif
	changeNotify(LOCALE_VIDEOMENU_VIDEOMODE, NULL);
	...
	changeNotify(LOCALE_VIDEOMENU_HUE, NULL);
...
bool CVideoSettings::changeNotify(const neutrino_locale_t OptionName, void *)
{
	...
	stringstream cmd;
	if(ARE_LOCALES_EQUAL(OptionName, LOCALE_VIDEOMENU_BRIGHTNESS))
	{
		printf("Changing brightness: %i\n",g_settings.brightness);
		cmd << "echo 1 " << g_settings.brightness <<" > //proc//videosettings";
		system (cmd.str().c_str());

	}
	else if(ARE_LOCALES_EQUAL(OptionName, LOCALE_VIDEOMENU_CONTRAST))
	{
		printf("Changing contrast: %i\n",g_settings.contrast);
		cmd << "echo 1 " << g_settings.contrast <<" > //proc//videosettings";
		system (cmd.str().c_str());
	}
	else if(ARE_LOCALES_EQUAL(OptionName, LOCALE_VIDEOMENU_HUE))
	{
		printf("Changing hue %i\n",g_settings.hue);
		cmd << "echo 4 " << g_settings.hue <<" > //proc//videosettings";
		system (cmd.str().c_str());
	}
	else if(ARE_LOCALES_EQUAL(OptionName, LOCALE_VIDEOMENU_SATURATION))
	{
		printf("Changing saturation: %i\n",g_settings.saturation);
		cmd << "echo 3 " << g_settings.saturation <<" > //proc//videosettings";
		system (cmd.str().c_str());
	}
	else if (ARE_LOCALES_EQUAL(OptionName, LOCALE_VIDEOMENU_43MODE))
	{
		printf("1.Changing scalingmode\n");
		videoDecoder->setAspectRatio(-2, g_settings.video_43mode);
	}
	else
	{
		...
		if ( g_settings.video_Mode == 0)
		{
			g_settings.screen_EndX = 640;
			g_settings.screen_EndY = 479;
		}
		else if ( g_settings.video_Mode == 1)
		{
			g_settings.screen_EndX = 720;
			g_settings.screen_EndY = 575;
		}
		else
		{
			g_settings.screen_EndX = 1280;
			g_settings.screen_EndY = 719;
		}
		...
		if(prev_video_mode != g_settings.video_Mode) {
			...
			if(ShowMsgUTF(LOCALE_MESSAGEBOX_INFO, g_Locale->getText(LOCALE_VIDEOMODE_OK), CMessageBox::mbrNo, CMessageBox::mbYes | CMessageBox::mbNo, "info.raw") != CMessageBox::mbrYes) {
				...
				{
					g_settings.screen_EndX = 640;
					g_settings.screen_EndY = 478;
				}
				else if ( g_settings.video_Mode == 1)
				{
					g_settings.screen_EndX = 720;
					g_settings.screen_EndY = 576;
				}
				else
				{
					g_settings.screen_EndX = 1280;
...
void CNeutrinoApp::InitMainMenu(CMenuWidget &mainMenu, CMenuWidget &mainSettings,  CMenuWidget &audioSettings, CMenuWidget &parentallockSettings,
				...
				CMenuWidget &service, CMenuWidget &fontSettings, CMenuWidget &audiopl_picSettings, CMenuWidget &streamingSettings, CMenuWidget &moviePlayer, CMenuWidget &shutdown)
{
	...
	//Enabled shutdown and added restart neutrino
	mainMenu.addItem(new CMenuForwarder(LOCALE_MAINMENU_SHUTDOWN, true, NULL, &shutdown, NULL, CRCInput::convertDigitToKey(shortcut++)));
	shutdown.addItem(new CMenuForwarder(LOCALE_SERVICEMENU_RESTART   , true, NULL, this, "restart", CRCInput::RC_1));
	shutdown.addItem(new CMenuForwarder(LOCALE_MAINMENU_REBOOT, true, NULL, this, "reboot", CRCInput::RC_2));
	shutdown.addItem(new CMenuForwarder(LOCALE_MAINMENU_SHUTDOWN, true, NULL, this, "shutdown", CRCInput::RC_3));
...
//	mainMenu.addItem( new CMenuForwarder(LOCALE_DBOXINFO, true, NULL, new CDBoxInfoWidget, NULL, CRCInput::convertDigitToKey(shortcut++)));
	mainMenu.addItem(new CMenuForwarder(LOCALE_SERVICEMENU_IMAGEINFO,  true, NULL, new CImageInfo(), NULL, CRCInput::convertDigitToKey(shortcut++)));
...
//		mainSettings.addItem(new CMenuForwarder(LOCALE_MAINSETTINGS_LCD       , true, NULL, &lcdSettings      , NULL, CRCInput::RC_9));
	...
	mainSettings.addItem(new CMenuForwarder(LOCALE_HDD_SETTINGS, true, NULL, new CHDDMenuHandler(), NULL, CRCInput::RC_9));
...
//		mtdexpert->addItem(new CMenuForwarder(LOCALE_FLASHUPDATE_READFLASHMTD , true, NULL, fe, "readflashmtd" ));
		//if(softupdate) //Opensat - Disabled this to allow writing mtd partitions
		...
		mainSettings.addItem(new CMenuForwarder(LOCALE_SERVICEMENU_UPDATE, true, NULL, updateSettings));
	}
...
void CNeutrinoApp::InitServiceSettings(CMenuWidget &service, CMenuWidget &scanSettings)
{
	...
	//service.addItem(GenericMenuSeparatorLine);
	//Removed here added to main menu
...
//Opensat: fix hardcoded option names
//	audioSettings.addItem(new CMenuOptionChooser(LOCALE_AUDIOMENU_ANALOGOUT, &g_settings.audio_AnalogMode, AUDIOMENU_ANALOGOUT_OPTIONS, AUDIOMENU_ANALOGOUT_OPTION_COUNT, true, audioSetupNotifier));
	...
	audioSettings.addItem(new CMenuOptionChooser("Dolby Digital on SPDIF", &g_settings.audio_spdif_dd, OPTIONS_RAW_SPDIF_OPTIONS, OPTIONS_RAW_SPDIF_OPTION_COUNT, true, audioSetupNotifier));
	audioSettings.addItem(new CMenuOptionChooser("DTS on SPDIF", &g_settings.audio_spdif_dts, OPTIONS_RAW_SPDIF_OPTIONS, OPTIONS_RAW_SPDIF_OPTION_COUNT, true, audioSetupNotifier));
	audioSettings.addItem(new CMenuOptionChooser("AAC on SPDIF", &g_settings.audio_spdif_aac, OPTIONS_RAW_SPDIF_OPTIONS, OPTIONS_RAW_SPDIF_OPTION_COUNT, true, audioSetupNotifier));
...
#if 0
	colorSettings_Themes.addItem(new CMenuForwarder(LOCALE_COLORTHEMEMENU_NEWGENERATION_THEME, true, NULL, this, "theme_newgeneration"));
#endif
}
