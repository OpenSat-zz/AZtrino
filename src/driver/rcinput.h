...
#define NUMBER_OF_EVENT_DEVICES 1
	...
	public:
		...
		enum
		{
			...
			RC_audio = KEY_AUDIO,
			RC_video = KEY_VIDEO,
			RC_tv = KEY_RECORD,//KEY_TV,
			RC_radio = KEY_STOP,//KEY_RADIO,
			RC_text = KEY_TEXT,
			RC_info = KEY_INFO,
			RC_epg	= KEY_EPG,
			RC_recall = KEY_LAST,
			...
			RC_sat = KEY_SAT,
			RC_sat2 = KEY_SAT2,
			RC_record	= KEY_RADIO,//KEY_RECORD,
			RC_play		= KEY_PAUSE,
			RC_pause	= KEY_PLAY,
			RC_forward	= KEY_FORWARD,
			RC_rewind	= KEY_REWIND,
			RC_stop		= KEY_TV,//KEY_STOP,
			...
			RC_nokey	= 0xFFFFFFFE,
			RC_channelup	= KEY_CHANNELUP,
			RC_channeldown	= KEY_CHANNELDOWN
		};
