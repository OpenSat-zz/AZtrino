...
#if AZBOX_GEN_1
		unsigned char* getFrameBufferPointer() const;
#else
...
#ifdef AZBOX_GEN_1
		bool loadPictureToMem       (const std::string & filename, const uint16_t width, const uint16_t height, const uint16_t stride,  unsigned char* const memp);
		bool loadPicture2Mem        (const std::string & filename,  unsigned char* const memp);
#else
