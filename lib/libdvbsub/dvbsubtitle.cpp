...
void cDvbSubtitleBitmaps::Draw()
{
	...
	int stride = CFrameBuffer::getInstance()->getStride();
...
#ifdef AZBOX_GEN_1
	unsigned char *sublfb = (unsigned char *)CFrameBuffer::getInstance()->getFrameBufferPointer();
#else
	...
	for (int i = 0; i < bitmaps.Size(); i++) {
		...
		for (int y2 = 0; y2 < bitmaps[i]->Height(); y2++) {
			...
			{
			int b = xoff + x2;
			//printf("doint x %d, y %d\n", b, y);
				int idx = *(bitmaps[i]->Data(x2, y2));
				//int idx = 9;
				//printf("IDX is %i\n", idx);
				*(sublfb + xoff + a + y) = save_colors[idx];
				*(sublfb + xoff + a + 1 + y) = save_colors[idx];
				*(sublfb + xoff + a + 2 + y) = save_colors[idx];
				*(sublfb + xoff + a + 3 + y) = save_colors[idx];
				*(sublfb + xoff + a + 4 + y) = save_colors[idx];
				*(sublfb + xoff + a + 5 + y) = save_colors[idx];
				*(sublfb + xoff + a + 6 + y) = save_colors[idx];
				*(sublfb + xoff + a + 7 + y) = save_colors[idx];
				a += 8;
			}
		}
		if(min_x > xoff)
		...
		if(min_y > ys)
			min_y = ys;
		if(max_x < (xoff + bitmaps[i]->Width()*8))
			max_x = xoff + bitmaps[i]->Width()*8;
